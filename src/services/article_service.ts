...
export class ArticleService {
  static async getAllArticles(query: ArticleQuery) {
    ...
    if (search) {
      paramCount++;
      conditions.push(
        `(a.title ILIKE $${paramCount} OR a.summary ILIKE $${paramCount} OR a.content ILIKE $${paramCount})`
      );
      values.push(`%${search}%`);
    }
    ...
    queryText += ` GROUP BY a.id, u.username, u.profile_picture_url, c.name`;
  ...
  static async getArticleById(id: string): Promise<Article | null> {
    const queryText = `
            ...
            GROUP BY a.id, u.username, u.profile_picture_url, c.name
        `;
  ...
  static async deleteArticle(id: string): Promise<boolean> {
    ...
    try {
      ...
      await client.query(
        "UPDATE comments SET deleted_at = CURRENT_TIMESTAMP WHERE article_id = $1",
        [id]
      );
  ...
  static async incrementViews(id: string): Promise<void> {
    await pool.query(
      "UPDATE articles SET views = views + 1 WHERE id = $1 AND deleted_at IS NULL",
      [id]
    );
  }
  ...
  static async getRecentArticlesByTimeFrame(
    days: number = 7
  ): Promise<Article[]> {
    const queryText = `
        SELECT 
